arch = '''import { motion } from 'framer-motion'
...
const Stage = ({ title, type, desc, delay }: any) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay }} className="flex flex-col items-center">
      <div className={w-[400px] p-5 rounded-lg border  bg-white shadow-sm flex items-center justify-between}>
        ...
        <div className={px-3 py-1.5 rounded-full flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wider  }>
